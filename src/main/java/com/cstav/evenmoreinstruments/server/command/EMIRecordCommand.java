...
public class EMIRecordCommand {
    ...
    private static int saveRecord(CommandContext<CommandSourceStack> stack) throws CommandSyntaxException {
        ...
        final Optional<ItemStack> record = CommonUtil.getItemInBothHands(target, ModItems.RECORD_WRITABLE);
        ...
        final CustomData channelTag = record.get().get(ModDataComponents.CHANNNEL);
        if (channelTag == null) {

            if (record.get().has(ModDataComponents.BURNED_MEDIA)) {
