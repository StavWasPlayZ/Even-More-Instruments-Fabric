...
public class WritableRecordItem extends EMIRecordItem {
    ...
    public boolean isBurned(final ItemStack stack) {
        return (stack.has(ModDataComponents.CHANNNEL) &&
            !stack.get(ModDataComponents.CHANNNEL).getUnsafe().getBoolean(WRITABLE_TAG))
                // May also be media burned
                || stack.has(ModDataComponents.BURNED_MEDIA);
    }
    ...
    public void onInsert(final ItemStack stack, final LooperBlockEntity lbe) {
        if (stack.has(ModDataComponents.BURNED_MEDIA))
            return;

        final CustomData channelRaw = stack.get(ModDataComponents.CHANNNEL);
        CompoundTag channel;
        ...
        stack.set(ModDataComponents.CHANNNEL, CustomData.of(channel));
    }
