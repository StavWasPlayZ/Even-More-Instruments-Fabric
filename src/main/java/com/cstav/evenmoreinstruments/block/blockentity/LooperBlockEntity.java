...
public class LooperBlockEntity extends BlockEntity {
    ...
    public void tick(Level pLevel, BlockPos pPos, BlockState pState) {
        ...
        for (final Tag pNote : channel.getList("notes", Tag.TAG_COMPOUND)) {
            ...
            try {
                ...
                pLevel.blockEvent(pPos, ModBlocks.LOOPER, 42, pitch);
    ...
    public static void onInstrumentPlayed(final InstrumentPlayedEvent.ByPlayer.ByPlayerArgs event) {
    ...
    public static void onPlayerLeave(final ServerGamePacketListenerImpl handler, MinecraftServer server) {
        ...
        RECORDING_LOOPERS.forEach((looper) -> {
            if (looper.lockedBy.equals(handler.player.getUUID())) {
                looper.reset();
